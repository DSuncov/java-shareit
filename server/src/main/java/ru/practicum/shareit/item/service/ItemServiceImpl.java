...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemResponseDto createItem(ItemCreateDto item, Long userId) {
        ...
        if (item.getRequestId() != null) {
            newItem.setRequest(itemRequestRepository.findById(item.getRequestId())
                    .orElseThrow(() -> new NotFoundException("Запроса с id = " + item.getRequestId() + " не существует.")));
        }
    ...
    public CommentResponseDto createComment(CommentCreateDto comment, Long authorId, Long itemId) {
        ...
        Booking booking = bookingRepository.findByAuthorAndItem(authorId, itemId)
                .orElseThrow(() -> new NotAvailableException("Пользователь с id = " + authorId + " не создавал бронирование."));
