...
import ru.practicum.shareit.exception.exceptions.AccessException;
...
public class BookingServiceImpl implements BookingService {
    ...
    public List<BookingResponseDto> getAllBookingsByOwnerAndState(Long ownerId, String state) {
        ...
        switch (state) {
            case "ALL" -> {
                bookings = bookingRepository.findByBookerIdAsc(ownerId);
            }
            case "CURRENT" -> {
                bookings = bookingRepository.findCurrentBookings(ownerId);
            }
            case "PAST" -> {
                bookings = bookingRepository.findPastBookings(ownerId);
            }
            case "FUTURE" -> {
                bookings = bookingRepository.findFutureBookings(ownerId);
            }
            case "WAITING", "REJECTED" -> {
                bookings = bookingRepository.requiredStatusBookings(ownerId, Status.valueOf(state));
            }
